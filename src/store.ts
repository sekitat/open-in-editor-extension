...
export async function setApiRoutesToStorage(urls: string[]) {
  const res = await Promise.all(urls.map((url) => searchRoutesByPath(url)));
  storedRoutes = [...storedRoutes, ...res.flatMap((r) => r)];
  const uniqueUrls = Array.from(
    new Map(storedRoutes.map((route) => [route.id, route])).values()
  );
  sessionStorage.setItem("urls", JSON.stringify(uniqueUrls));
}
